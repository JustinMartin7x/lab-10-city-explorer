...
const { mungeLocation, mungeWeather, mungeTrails } = require('../ustil');
...
app.get('/weather', async (req, res) => {
  try {
    const WeatherURL = `https://api.weatherbit.io/v2.0/forecast/daily?lat=${req.query.latitude}&lon=${req.query.longitude}&key=${process.env.WEATHER_KEY}`;
    const data = await request.get(WeatherURL);
...
app.get('/reviews', async (req, res) => {
  try {
    ...
    const data = await request.get(URL)
      .set('Authorization', process.env.YELP_KEY);
    const newData = mungeReview(data.businesses);
    res.json(newData);
  } catch (e) {
    res.json({ error: e.message });
  }
